...
function swtGetTestListCode2(_apiCompTag, _gpuCompTag, _sysCompTag, _graphListTag)
{
    ...
    function(data,status) 
    {
        ...
        if (json.errorCode == "1")
        {
            ...
            var sysListCode = "<legend><input id=\"sysCompCheck\" name=\"sysCompCheck\" type=\"checkbox\" onchange=\"swtCheckTestInfoOption('sysCompCheck');\" checked=\"checked\"></input> System Comparison</legend>";
            ...
            $("#sysNum").val(json.uniqueCardNameList.length);
...
{
    swtGetTestData2(event.data.testPosID, ui.value);
    //console.log("evt: " + event.type);
    //console.log("pos: " + ui.value);
}
...
function swtIncDecSubTest(_tag, _testPosID, _step)
{
    ...
    swtGetTestData2(_testPosID, nextVal);
}

function swtGetTestData2(_testPosID, _subTestPosID)
{
    //var t1 = $("#cardList").val();
    //var t2 = $("#testList").val();
    ...
    function(data,status) 
    {
        ...
        if (json.errorCode == "1")
        {
            ...
            var curCardPosIDMap = {};
            for (var i = 0; i < json.uniqueCardIDList.length; i++)
            {
                curCardPosIDMap[json.uniqueCardIDList[i]] = i;
            }
                    ...
                    for (var j = 0; j < umdNum; j++)
                    {
                        if (curDriverPosIDMap[j])
                        {
                                ...
                                (json.resultIDList[0][n1] < 200000000))
                            {
                                ...
                                var t1 = json.uniqueCardNameList[cardPosID] + "-" + systemName + "-" + json.uniqueDriverNameList[j];
                                var testValSec = { type:          "line",
                                                   lineThickness: 3,
                                                   axisYType:     "primary",
                                                   showInLegend:  true,
                        ...
                        for (var j = 0; j < umdNum; j++)
                        {
                            if (curDriverPosIDMap[j])
                            {
                                    ...
                                    (json.resultIDList[i][n1] < 200000000))
                                {
                                    ...
                                    console.log("1: " + testVal.length);
                                    console.log("2: " + (usedCardNum * curDriverPosIDList.length + usedDriverNum));
                                    console.log("3: " + i);
                                    console.log("4: " + usedCardNum);
                                    console.log("5: " + curDriverPosIDList.length);
                                    console.log("6: " + usedDriverNum);
                                    
                                    testVal[usedCardNum * curDriverPosIDList.length + usedDriverNum].dataPoints.push(tmpDataSec);
