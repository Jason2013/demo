...
function swtFeedData($_db, $_subTestList, $_dataList, $_testName, $_noiseDataID, $_noiseDataNum)
{
    ...
    if (strlen($_dataList) > 0)
    {
        ...
        $sql1 = "LOAD DATA LOCAL INFILE \"" . $tmpPathName . "\" IGNORE INTO TABLE tmp_table_test_data1 " .
                ...
                "test_case_id, group_name, frame_id, verify_status);";
        if ($db->QueryDBNoResult($sql1) == null)
        {
            $returnMsg["errorCode"] = 0;
        ...
        $sql1 = "SELECT t0.result_id, t1.test_id, " .
                "t0.data_value1, t0.data_value2, t0.data_value3, t0.data_value4, t0.test_case_id, t2.test_id, t0.frame_id, t0.verify_status " .
        ...
        while ($row1 = $db->FetchResult())
        {
            ...
            $frameID = intval($row1[8]);
            $verifyStatus = intval($row1[9]);
            ...
            //$tmpFileDataNum = intval($tmpFileSize / ($tmpDataSize * 4 + $tmpDataIDSize * 3));
            //
            ////$n1 = $testCaseID - $tmpFileDataNum;
            //$n1 = $frameID - $tmpFileDataNum;
            //for ($i = 0; $i < $n1; $i++)
            //{
            ...
            //fseek($tmpFileHandle, $frameID * ($tmpDataSize * 4 + $tmpDataIDSize * 3), SEEK_SET);
            $t1 = pack("iddddiiii", $tmpSubTestID, $dataValue1, $dataValue2, $dataValue3, $dataValue4, 
                       $testCaseID, $groupID, $frameID, $verifyStatus);
            fwrite($tmpFileHandle, $t1);
        }
        if ($tmpFileHandle != false)
        {
...
function swtParseLogFile($_pathName, $_machineID, $_compilerName, $_noiseDataID, $_noiseDataNum)
{
    ...
    global $db_mis_table_create_string005;
    ...
    while ($data = fgetcsv($handle, 0, ","))
    {
        ...
        $tmpName = $data[0];
        $isTestCasesShow = $data[1] == "TestCase";
            ...
            ($isTestCasesShow == true))
        {
            ...
            if (($tmpTestID - 1) >= $curTestID)
            {
                ...
                for ($i = 0; $i < 2; $i++)
                {
                    // Time[Execution](ms), Time[Record](ms)
    ...
    while ($data = fgetcsv($handle, 0, ","))
    {
        ...
        else
        {
            if (strlen($tmpName) > 0)
            {
                ...
                $fullTestName = str_replace("-", "_", $fullTestName);
                
                $tmpTestName = str_replace(" ", "", $fullTestName);
                $tmpTestName = cleaninput($tmpTestName, 256);
                // create test table
                //$sql1 = sprintf($db_mis_table_create_string005, cleaninput($fullTestName, 256));
                $sql1 = sprintf($db_mis_table_create_string005, $tmpTestName);
                $params1 = array();
                if ($db->QueryDB($sql1, $params1) == null)
                {
                    fclose($handle);
