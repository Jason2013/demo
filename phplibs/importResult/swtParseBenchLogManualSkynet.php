    ...
    while ($data = fgetcsv($handle, 0, ","))
    {
        ...
        if (strlen($tmpName) > 0)
        {
            ...
            for ($i = 1; $i < count($data); $i++)
            {
                ...
                else if (strpos($data[$i], "(") !== false) // CSLatency
                {
                    // data column id
                    $dataKeyDataColumnID = $i;
                    $subTestNameFilterNum = $dataKeyDataColumnID - 1;
...
$cardFolderList = array($logStoreDir . "/" . $logFolderName . "/" . $machineFolder);
...
if ($resultFileNum == 0)
{
    ...
    foreach ($cardFolderList as $tmpCardPath)
    {
        ...
        foreach ($noiseFolderList as $tmpPath)
        {
            ...
            $returnMsg["pieceFolderList"] = $pieceFolderList;
            ...
            else
            {
                ...
                $pieceFolderList = array($tmpPath . "/" . $resultTestName . "_" . $resultApiName);
                $returnMsg["pieceFolderList"] = $pieceFolderList;
