...
include_once "swtClassGenFlatDataVer3.php";
...
$allFileFolderTestNameList = $returnSet["allFileFolderTestNameList"];
$allFileReportUmdNameList = $returnSet["allFileReportUmdNameList"];
$allFileTestCaseUmdDataMaskList = $returnSet["allFileTestCaseUmdDataMaskList"];

$cardNameList = $returnSet["cardNameList"];
$machineIDList = $returnSet["machineIDList"];
$allFolderList = $returnSet["allFolderList"];
...
if (count($allFileList) == 0)
{
    ...
    $allFileFolderTestNameList = array();
    $allFileReportUmdNameList = array();
    $allFileTestCaseUmdDataMaskList = array();
    foreach ($allFileList as $tmpName)
    {
        // set for test pos list for all result files
        array_push($allFileTestPosList, array());
        array_push($allFileTestCaseNumList, array());
        array_push($allFileSubTestNameFilterNumMaxList, array());
        array_push($allFileFolderTestNameList, array());
        array_push($allFileReportUmdNameList, array());
        array_push($allFileTestCaseUmdDataMaskList, array());
    }
}
...
if ($fileID <= count($uniqueCardNameList))
{
    ...
    $returnSet = $flatDataGen->getTestStartPos($curCardNameList);
    $testStartPosList = $returnSet["testStartPosList"];
    $testCaseNumList = $returnSet["testCaseNumList"];
    $subTestNameFilterNumMaxList = $returnSet["subTestNameFilterNumMaxList"];
    $folderTestNameList = $returnSet["folderTestNameList"];
    ...
    if ($curPairMachineID != -1)
    {
        ...
        $returnSet = $flatDataGen->getTestStartPos($pairCardNameList);
        $pairTestStartPosList = $returnSet["testStartPosList"];
        $pairTestCaseNumList = $returnSet["testCaseNumList"];
        $pairSubTestNameFilterNumMaxList = $returnSet["subTestNameFilterNumMaxList"];
        $pairFolderTestNameList = $returnSet["folderTestNameList"];
        $pairReportUmdNameList = $returnSet["reportUmdNameList"];
        $pairTestCaseUmdDataMaskList = $returnSet["testCaseUmdDataMaskList"];
    }
    
    for ($i = 0; $i < count($curCardNameList); $i++)
    {
        $allFileTestPosList[$curCardNameList[$i]] = $testStartPosList[$i];
        $allFileTestCaseNumList[$curCardNameList[$i]] = $testCaseNumList[$i];
        $allFileSubTestNameFilterNumMaxList[$curCardNameList[$i]] = $subTestNameFilterNumMaxList[$i];
        $allFileFolderTestNameList[$curCardNameList[$i]] = $folderTestNameList[$i];
        $allFileReportUmdNameList[$curCardNameList[$i]] = $reportUmdNameList[$i];
        $allFileTestCaseUmdDataMaskList[$curCardNameList[$i]] = $testCaseUmdDataMaskList[$i];
    }
    for ($i = 0; $i < count($pairCardNameList); $i++)
    {
        $allFileTestPosList[$pairCardNameList[$i]] = $pairTestStartPosList[$i];
        $allFileTestCaseNumList[$pairCardNameList[$i]] = $pairTestCaseNumList[$i];
        $allFileSubTestNameFilterNumMaxList[$pairCardNameList[$i]] = $pairSubTestNameFilterNumMaxList[$i];
        $allFileFolderTestNameList[$pairCardNameList[$i]] = $pairFolderTestNameList[$i];
        $allFileReportUmdNameList[$pairCardNameList[$i]] = $pairReportUmdNameList[$i];
        $allFileTestCaseUmdDataMaskList[$pairCardNameList[$i]] = $pairTestCaseUmdDataMaskList[$i];
    }
...
$valueSet["allFileFolderTestNameList"] = $allFileFolderTestNameList;
$valueSet["allFileReportUmdNameList"] = $allFileReportUmdNameList;
$valueSet["allFileTestCaseUmdDataMaskList"] = $allFileTestCaseUmdDataMaskList;

$valueSet["cardNameList"] = $cardNameList;
$valueSet["machineIDList"] = $machineIDList;
$valueSet["allFolderList"] = $allFolderList;
