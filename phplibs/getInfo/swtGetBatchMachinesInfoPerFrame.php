...
$sql1 = "SELECT t0.machine_id, t1.card_id, t2.env_name, t3.env_name, t4.env_name " .
        "FROM mis_table_result_list t0 " .
        "LEFT JOIN mis_table_machine_info t1 " .
        "USING (machine_id) " .
        "LEFT JOIN mis_table_environment_info t2 " .
        ...
        "ON (t4.env_type=4 AND t4.env_id=t0.umd_id) " .
        "WHERE t0.batch_id=? ORDER BY t0.umd_id ASC";
...
while ($row1 = $db->fetchRow())
{
    ...
    array_push($driverNameList, $row1[4]);
}
