...
def main():
    ...
    for tmpPath1 in tmpFolderList1 :
        ...
        for tmpPath2 in tmpFolderList2 :
            ...
            if (len(tmpFolderList3) == 0) :
                tmpPathName = os.path.join(tmpPath2, "*.txt")
                tmpFileList = glob.glob(tmpPathName)
                for tmpPath4 in tmpFileList :
                    if (os.path.exists(tmpPath4)) :
                        for tmpLine in open(tmpPath4) :
                            fileHandle.writelines(tmpLine)
                        fileHandle.write("\n")
            else :
                for tmpPath3 in tmpFolderList3 :
                    tmpPathName = os.path.join(tmpPath3, "*.txt")
                    tmpFileList = glob.glob(tmpPathName)
                    for tmpPath4 in tmpFileList :
                        if (os.path.exists(tmpPath4)) :
                            for tmpLine in open(tmpPath4) :
            ...
            if (os.path.exists(tmpDestRunLogFileName) == False) :
                fileHandle2 = open(tmpDestRunLogFileName, "w")
                fileHandle2.close()
            fileHandle.close()
