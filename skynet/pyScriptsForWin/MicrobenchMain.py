...
#Global Variables
BENCHMARK_DIR = "C:\\Microbench\\forSkynet\\Windows";
LogFile="";
GENLOG=""
...
MACHINE_CONFIG = "C:\\Microbench\\machineConfig\\machine_info.json"
DEFAULT_CONFIG = "C:\\Microbench\\machineConfig\\default_info.json"
CLINFO_CONFIG = "C:\\Driver_Details\\CLInfo.txt"
ZIP_FOLDER_NAME = "C:\\Microbench\\temp"
...
def main():
    ...
    parser.Check_Result()
    parser.Parse(Get_Json(CLINFO_CONFIG), CL_Status)
    '''

def Prep():
    print("Checking Preconditions ... ");
    if(os.path.isfile("Logscript.txt")):
        os.system('del Logscript.txt')
    LogFile = open("Logscript.txt",'w');
    LogFile.write("************Logging started **********\n")
    if(os.path.isfile(BENCHMARK_DIR+"\\default.json")):
        LogFile.write("************Configuration File Found *************\n")
    else:
        LogFile.write("!!!! Configuration File not found for benchmark ... Existing ... \n")
        exit(0)
    if(os.path.isfile(GENLOGFILE)):
        os.system('del '+GENLOGFILE)
