...
tmpPathList = glob.glob(os.path.join("/testres/MISTestSkynet", "*.txt"))
...
tmpPath = os.path.join("/MIS/json", "machine_info.json")
...
tmpPath = os.path.join("/MIS/json", "default_info.json")
if os.path.isfile(tmpPath):
    f.write(tmpPath, os.path.basename(tmpPath))
else:
    print tmpPath, "is missing, line: ", __line__()
